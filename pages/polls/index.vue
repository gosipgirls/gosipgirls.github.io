<template>
  <div class="col-span-5 lg:col-span-4">
    <div class="w-full h-full flex flex-col py-5 px-2 rounded *:transition-all">
      ...
      <div
        class="flex justify-between my-4 border-t border-t-gray-600 pt-4 *:transition-all"
      >
        <button
          :disabled="!meta.prev"
          :class="`${
            meta.prev ? 'prev' : ' cursor-not-allowed'
          } blur-background  px-5 py-1`"
          @click="prev"
        >
          &lt; Previous
        </button>
        <button
          :disabled="!meta.next"
          :class="`${
            meta.next ? 'next' : ' cursor-not-allowed'
          } blur-background  px-5 py-1`"
          @click="next"
        >
          Next >
